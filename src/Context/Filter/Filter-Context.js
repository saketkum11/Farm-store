...
const FilterProvider = ({ children }) => {
  ...
  const getFilteredData = (sortedData, fruit, vegetable, eggAndMeat, grain) => {
    ...
    if (fruit === true) {
      getFruit = sortedData.filter(
        (product) => product.categoryName === 'fruit'
      );
    }
    if (grain === true) {
      getGrain = sortedData.filter(
        (product) => product.categoryName === 'grain'
      );
    }
    if (eggAndMeat === true) {
      getEggAndMeat = sortedData.filter(
        (product) => product.categoryName === 'egg and meat'
      );
    }
    if (vegetable === true) {
      getVegetable = sortedData.filter(
        (product) => product.categoryName === 'vegetable'
      );
    }

    return [...getFruit, ...getEggAndMeat, ...getGrain, ...getVegetable];
  };
  ...
  return (
    <filterContext.Provider value={{ state, dispatch, sortPriceData }}>
      {children}
